...
MouseMotionListener, ActionListener {
	 ...
	 * @param _what whether enabled or disabled.
	 */
        ...
        if (_event.getSource().equals(View.getInstance().getJbtn_exit())) {
            ...
            case Constants.CONTROL_PATINING_INDEX_PAINT_1:
                ...
                Picture.getInstance().changePaintObject(new Point(
                        (_event.getX() - Page.getInstance()
                        .getJpnl_toMove().getX())
                        * Status.getImageSize().width 
                        / Status.getImageShowSize().width, 
                        (_event.getY() - Page.getInstance()
                        .getJpnl_toMove().getY())
            ...
            case Constants.CONTROL_PAINTING_INDEX_SELECTION_CURVE:
                ...
                Picture.getInstance().changePen(new PenSelection(
                        Constants.PEN_ID_LINES, 1, Color.gray));
                Picture.getInstance().addPaintObjectWrinting();
                break;
	...
	public void mouseDragged(final MouseEvent _event) {
		...
		if (_event.getModifiersEx() ==  leftMouse && (_event.getSource()
		        .equals(Page.getInstance().getJlbl_painting()))) {
    ...
    @Override public void mouseReleased(final MouseEvent _event) {
    	...
    	if (_event.getSource().equals(View.getInstance().getJbtn_exit())) {
    	...
    	} else if (_event.getSource().equals(Paint.getInstance().getTb_new()
                .getActionCause())) {
    	    actionNew();
        } else if (_event.getSource().equals(Paint.getInstance().getTb_save()
                .getActionCause())) {

            actionSave();
        }  else if (_event.getSource().equals(Paint.getInstance().getTb_load()
                .getActionCause())) {
        ...
        } else if (_event.getSource().equals(Paint.getInstance().getTb_zoomOut()
    	        .getActionCause())) {
            ...
            Page.getInstance().getJpnl_toMove().setLocation(
                    (oldLocation.x) / 2, 
                    (oldLocation.y) / 2);
        ...
        } else if (_event.getSource().equals(Paint.getInstance().getTb_copy()
                .getActionCause())) {
        ...
        } else if (_event.getSource().equals(Paint.getInstance().getTb_paste()
                .getActionCause())) {
        ...
        } else if (_event.getButton() == 1 &&  _event.getSource().equals(
                Page.getInstance().getJlbl_painting())) { 
            ...
            case Constants.CONTROL_PAINTING_INDEX_ZOOM:
                ...
                * ViewSettings.ZOOM_MULITPLICATOR;
                ...
                newY = Math.max(newY, -(Status.getImageShowSize().height
    ...
    private void actionPipette(final MouseEvent _event) {
        ...
        final double dX = hundred 
                * (0.000 + Status.getImageSize().width)
                / Status.getImageShowSize().width;
        final double dY = hundred 
                * (0.000 + Status.getImageSize().height)
                / Status.getImageShowSize().height;
        
        int color =  Picture.getInstance().getColorPX((int)
                ((_event.getX() - Page.getInstance().getJlbl_painting()
                        .getX()) * hundred / dX), (int)
                ((_event.getY() - Page.getInstance().getJlbl_painting()
                        .getY()) * hundred / dY));
    ...
    private void actionSelectionLine(final MouseEvent _event) {
        ...
        BufferedImage bi_selection = new BufferedImage(
                Status.getImageSize().width, Status.getImageSize()
        ...
        while (po_current != null 
            ...
            if (po_current.isInSelectionImage(r_sizeField)) {
                    ...
                    if (r_sizeMax == null) {
                        ...
                        r_sizeMax = new Rectangle(
                                Math.min(r.x, r_sizeMax.x),
                                Math.min(r.y, r_sizeMax.y),
                                Math.max(r.width + r.x, 
                                        r_sizeMax.width),
                                Math.max(r.height + r.y,
                                        r_sizeMax.height));
                    }
...
//                         .getJlbl_painting().getHeight());
        ...
        if (r_sizeMax != null) {
            ...
            Rectangle realRect = new Rectangle(
                    r_sizeMax.x, r_sizeMax.y, 
                    r_sizeMax.width - r_sizeMax.x, 
                    r_sizeMax.height - r_sizeMax.y);
